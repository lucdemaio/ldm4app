...
// Use global THREE when importing as module fails due to CDN export differences
const THREE_GLOBAL = (typeof window !== 'undefined' && window.THREE) ? window.THREE : (typeof THREE !== 'undefined' ? THREE : null);
if (!THREE_GLOBAL) console.warn('PMREMGenerator: global THREE not found — ensure Three.js is loaded before PMREMGenerator');
const { DoubleSide, GammaEncoding, LinearEncoding, LinearFilter, LinearToneMapping, Mesh, NearestFilter, NoBlending, OrthographicCamera, PlaneGeometry, Scene, ShaderMaterial, WebGLRenderTargetCube, sRGBEncoding } = (typeof window !== 'undefined' && window.THREE) ? window.THREE : (typeof THREE !== 'undefined' ? THREE : {});
...
var PMREMGenerator = ( function () {
	...
	var scene = new Scene();
	var planeMesh = new Mesh( new PlaneGeometry( 2, 2, 0 ), shader );
