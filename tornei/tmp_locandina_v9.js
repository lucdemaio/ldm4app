    ...
    generateLocandinaClassifica: async function (torneoNome, classifica) {
        try {
            ...
            for (let index = 0; index < top5.length; index++) {
                ...
                // Draw number on an offscreen canvas then composite it to avoid being overwritten
                ...
                offCtx.strokeText(number, off.width/2, off.height/2);
                // Blue fill
                offCtx.fillStyle = '#0f4b8f';
                offCtx.fillText(number, off.width/2, off.height/2);

                // Composite onto main canvas centered on badge
                ctx.drawImage(off, 110, y + 20, 80, 80);

                // Debug: log pixels from offscreen and main canvas immediately after compositing
                try {
                    const offPixel = offCtx.getImageData(off.width/2, off.height/2, 1, 1).data;
                    console.log('[locandina-debug] offscreen number', number, 'pixel RGBA:', offPixel);
                } catch (e) { console.warn('[locandina-debug] offscreen getImageData failed', e); }
                try {
                ...
                console.log('[locandina-generator] composited number', number, 'for index', index+1);
            ...
            // Debug: sample pixel color at each number center and log RGBA
