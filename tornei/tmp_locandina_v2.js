    ...
    generateLocandinaClassifica: async function (torneoNome, classifica) {
        try {
            ...
            top5.forEach((squadra, index) => {
                ...
                // Position badge
                // Make ranks 1, 2 and 3 blue like rank 4 per request
                const badgeColors = ['#667eea', '#667eea', '#667eea', '#667eea', '#667eea'];
                ctx.fillStyle = badgeColors[index];
                ctx.beginPath();
                ctx.arc(150, y + 60, 40, 0, Math.PI * 2);
                ctx.fill();

                // Number: ensure visible blue with white stroke for contrast
                ctx.save();
                ...
                ctx.strokeText((index + 1).toString(), 150, y + 75);
