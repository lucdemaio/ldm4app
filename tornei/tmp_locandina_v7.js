    ...
    generateLocandinaClassifica: async function (torneoNome, classifica) {
        try {
            ...
            for (let index = 0; index < top5.length; index++) {
                ...
                // Draw number directly on canvas centered on badge for consistent color
                const number = (index + 1).toString();
                ctx.save();
                // Ensure normal blending mode so fill actually covers the stroke area
                ...
                ctx.strokeText(number, 150, y + 60);
                // Blue fill as requested
                ctx.fillStyle = '#0f4b8f';
                ctx.fillText(number, 150, y + 60);
                ctx.restore();
